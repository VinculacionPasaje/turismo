...
@section('contenido')
                                ...
                                @foreach($galerias as $noticia)
                                 @if($noticia->estado !=0)
                                    <tr data-id="{{$noticia->id}}">
                                        ...
                                        <td>
                                            {!!link_to_route('galeriasHospedajes.edit', $title = 'Editar', $parameters = $noticia->id, $attributes = ['class'=>'btn  btn-primary btn-sm'])!!}
    ...
    {!! Form::open(['route' => ['galeriasHospedajes.destroy', ':USER_ID'], 'method' => 'DELETE', 'id' => 'form-delete']) !!}
    {!! Form::close() !!}
@endsection
@section('script')
