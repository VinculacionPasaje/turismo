...
class TodasActividadesController extends Controller
{
    public function edit($id){
        $todos = TodasActividades::find($id);
        ...
        return view('administracion.todos.edit',compact('comentariosNoticias','total','comentariosActividades','comentariosAlimentacion','comentariosAtractivosT', 'comentariosHospedaje', 'comentariosDiversion', 'comentariosEventos','todos'));


    }

    public function index(Request $request){
        $todos= TodasActividades::where('id',1)->paginate(1);
         $comentariosAtractivosT = ComentariosAtractivosT::where('aprovado',0)->count();
        $comentariosHospedaje = ComentariosHospedaje::where('aprovado',0)->count();
        $comentariosDiversion = ComentariosDiversion::where('aprovado',0)->count();
        $comentariosEventos = ComentariosEventos::where('aprovado',0)->count();
        ...
        return view('administracion.todos.index',compact('comentariosNoticias','total','comentariosActividades','comentariosAlimentacion','comentariosAtractivosT', 'comentariosHospedaje', 'comentariosDiversion', 'comentariosEventos','todos'));

    }

     public function update(TodosRequest $request, $id){
        $todos = TodasActividades::find($id);
        $todos->fill($request->all());

        if($todos->save()){
            return Redirect::to('administracion/todos')->with('mensaje-registro', 'Contenido Actualizado Correctamente');
        }
